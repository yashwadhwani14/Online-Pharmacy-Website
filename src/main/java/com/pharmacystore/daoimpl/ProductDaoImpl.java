...
public class ProductDaoImpl implements ProductDao {
	...
	public boolean deleteProduct(int productid) {
		try (Connection con = DbConnection.getDatabaseConnection())
		{
			...
			PreparedStatement pst = con.prepareStatement("DELETE FROM product WHERE " 
					+ " productid = ?");
	...
	public boolean updateProduct(Product product) {
		try (Connection con = DbConnection.getDatabaseConnection())
		{
			...
			PreparedStatement pst = con.prepareStatement("UPDATE product SET " 
					+ " productname = ? ,price = ? , " 
...
List<Product> productList = new ArrayList<>();
	...
	@Override
	public List<Product> displayProductCategoryWise(int categoryid) {
		List<Product> productList = new ArrayList<>();
		try (Connection con = DbConnection.getDatabaseConnection()) 
		{
	...
	public int getQuantityOfProduct(int pid) {
		try (Connection con = DbConnection.getDatabaseConnection()) 
		{
			...
			PreparedStatement pst = con.prepareStatement("SELECT quantity FROM product"
					+ "WHERE productid=?");
	...
	public boolean updateQuantityAfterCancellation(int pid, int quantity) {
		try (Connection con = DbConnection.getDatabaseConnection()) 
		{
			...
			pst = con.prepareStatement("UPDATE product SET quantity =?"
					+ "WHERE productid=?");
			pst.setInt(1, prev_quantity+ quantity);
			pst.setInt(2, pid);
